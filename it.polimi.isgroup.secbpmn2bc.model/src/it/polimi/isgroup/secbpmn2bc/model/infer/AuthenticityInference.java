...
public class AuthenticityInference implements SecurityAnnotationInference {
	...
	private List<Combination> getTaskCombinations(Task task){
		...
		if(task.getType() != TaskType.MANUAL && task.getType() != TaskType.USER) {
			...
			result.add(new Combination(true, BlockchainType.PUBLIC, false, Enforcement.POSSIBLE, GlobalEnforcement.POSSIBLE.getValue()));
			result.add(new Combination(true, BlockchainType.PUBLIC, true, Enforcement.POSSIBLE, GlobalEnforcement.POSSIBLE.getValue()));
